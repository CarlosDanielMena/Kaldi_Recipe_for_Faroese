#!/usr/bin/env bash
#--------------------------------------------------------------------#
# Copyright   2022 Reykjavik University (Author: Carlos Daniel 
#Hernández Mena - carlosm@ru.is)

#--------------------------------------------------------------------#
#If you are in Ubuntu, you will need to install this:
...
#--------------------------------------------------------------------#
#If you are running this recipe without slurm, you will probably
#need to type this in the terminal:
# $ sudo nvidia-smi -c 3
...
#Download the RAVNURSSON Corpus using this link: http://hdl.handle.net/20.500.12537/276
#and unzip it in your system.
corpus_root=/<path-to-ravnursson-corpus>/RAVNURSSON

prondict_orig=local/etc/FAROESE_ASR.dic
arpa_lm_3g=local/etc/3GRAM_ARPA_MODEL_PRUNED.lm
arpa_lm_4g=local/etc/4GRAM_ARPA_MODEL.lm
...
if  [ $current_stage -ge $from_stage ] && [ $current_stage -le $to_stage ]; then
    ...
    python3 local/corpus_faroese_data_prep.py $corpus_root
...
if  [ $current_stage -ge $from_stage ] && [ $current_stage -le $to_stage ]; then
    echo "-----------------------------------"
    echo "Stage $current_stage: Aligning the train_10 set to tri1"
    echo "-----------------------------------"
    steps/align_si.sh \
    --nj $nj_train \
    --cmd "$train_cmd" \
